...
class CrashTest(testbase.TestCase):
    ...
    def run_test(self):
        context.current_testresult().log_record(
            EnumLogLevel.APPCRASH,
            "App Crash",
            attachments={"QQ12e6.dmp": __file__, "QQ12e6.txt": __file__},
        )
...
class ResmgrTest(testbase.TestCase):
    ...
    def pre_test(self):
        ...
        self.file_name = "a_%s%s_resmgr.txt" % (
            sys.version_info[0],
            sys.version_info[1],
        )
        self.link_file_name = "readme_%s%s.txt.link" % (
            sys.version_info[0],
            sys.version_info[1],
        )
        with codecs_open(
            os.path.join(self.resource_root, self.file_name), "w", encoding="utf-8"
        ) as f:
            f.write("abc")
        with codecs_open(
            os.path.join(self.resource_root, self.link_file_name), "w", encoding="utf-8"
        ) as f:
    ...
    def _test_getfile(self, resmgr):
        ...
        with codecs_open(
            resmgr.get_file(self.link_file_name[:-5]), encoding="utf-8"
        ) as f:
            print(f.read())
