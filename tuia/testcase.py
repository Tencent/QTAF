...
def _screenshot(path):
    '''save screenshot
    '''
    #11/04/28 allenpan    创建
    #11/05/25 aaronlai    CreateCompatibleBitmap在超时时可能会失败，修改实现
    #11/05/27 aaronlai    捕获IOError
    #11/06/03 allenpan    用回原来的实现，并加上try except Exception
    #11/06/20 aaronlai    利用剪贴板截屏，解决不能截获popupmenu之类的窗口的问题
    #11/06/20 aaronlai    本日脚本出现非常多的截图失败情形，打log看看
    #11/06/20 aaronlai    利用剪贴板截屏存在较严重的问题，暂时屏蔽
    #11/06/21 allenpan    用captureblt属性解决不能截获menu的问题
    #11/06/21 aaronlai    修正从剪贴板获取截屏的问题，但仍先屏蔽
    #11/10/20 aaronlai    把截图失败的原因打入log
    #12/01/10 aaronlai    修正CreateCompatibleBitmap引起的截图失败问题
    #13/08/23 aaronlai    修改获取Bitmap数据的方式，在特定分辨率下会失败的问题
