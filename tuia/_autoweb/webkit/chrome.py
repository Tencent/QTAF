...
class WebPage(_webkit.WebPage):
    ...
    #2012-08-14    shadowyang  使用Chrome测试桩
    ...
    def _get_parent_hwnd(self):
        ...
        #2014/02/21 miawu   为了兼容不同版本的chrome窗口结构不一致的情况，需要遍历各个版本的窗口路径
    ...
    def _get_page_hwnd(self):
        '''根据pid查找页面的窗口句柄'''
        #2014/03/12 miawu   创建，为了兼容不同版本的chrome窗口结构不一致的情况，需要遍历各个版本的窗口路径
        #旧版本页面作为一个独立的窗口
        '''
    ...
    def openUrl(url):
        ...
        @return: 新打开的页面
        '''
        #2012/12/6 shadowyang 改为CreateProcess创建进程，Popen方式与Staf有冲突
        path = WebPage._get_browser_path()
        cmdline = [path, url]
        port = 9200
		
        #2014/4/21 shadowyang 检测是否存有一个chrome & chromespy实例
        processid = 0;
        try:
        ...
        #2014/4/21 terisli 如果存在原有spy进程，则使用原port
        if processid <= 0:
    ...
    def HWnd(self):
        ...
        #2013/03/12 miawu       页面所在窗口句柄存储在_page_hwnd
